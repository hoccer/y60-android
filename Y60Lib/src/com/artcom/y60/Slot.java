...
/**
 * Represents a UI element the user can interact with in a simple fashion, such
 * as
 * 
 * <ul>
 ...
 * </ul>
 * 
 * The purpose the slot API to provide decoupling between client (application)
 * code which defines the interaction options for the user (e.g. to shift a
 * video to TV) and code providing the needed functionality (e.g. send a play
 * command to the TV). Additionally, the slot API allows both (client as well as
 * provider) to abstract from details of UI primitives (clicking, tapping,
 * etc.). Decoupling is achieved by using a {@link SlotHolder}, which acts as a
 * bridge between both sides: a slot holder is initialized and passed to the
 * client, which uses it to e.g. activate and deactivate it for interaction;
 * slots use the holder to update (invalidate) the application view.
 * 
 * From a provider perspective, a slot is basically made up of two objects:
 * 
 * <ul>
 * <li>a {@link SlotLauncher} providing implementation for gaining/losing
 * focusing and for <i>launching</i> this slot, i.e. clicking it or
 * drag-and-dropping something on it, such as firing an Intent to launch an
 * Activity or just making a simple API to somewhere.</li>
 * <li>a {@link SlotViewer} providing implementation for viewing this slot, i.e.
 * retrieving a {@link android.view.View} on this slot</li>
 * </ul>
 * 
...
public class Slot {
    ...
    /**
     * Is used to launch whatever the slot is supposed to do, and to react to
    ...
    /**
     * Activates this slot for interaction, i.e. launch is executed when the
     * view is clicked.
     */
    ...
    public boolean isOnFocus(View pThumbView) {
        ...
        if (view.getLeft() + PADDING_X < x && view.getRight() - PADDING_X > x
                && view.getBottom() > y) {
